...
\title{ScholarLM: An AI-Assisted PDF Study Workspace}
...
\item intent-aware explanations that keep the written solution separate from the spoken teaching narrative;
...
The Explain workflow supports selected PDF text, canvas text, handwriting, and screenshots. Before composing a response, the model classifies the selection as theory, mathematics, problem solving, or general material. This small routing step lets the system adjust its teaching style to the task. A theory passage receives a conceptual explanation, while a mathematical selection keeps equations, substitutions, working steps, and the final result in the written answer. Word problems and other multi-step tasks receive a more deliberate solution structure.

Each response contains two related but distinct fields. The \texttt{answer} field is shown in the explanation panel and is the only generated content copied to a canvas text box or sticky note. The \texttt{voiceExplanation} field is written as a natural teacher-to-student account and is reserved for speech. Separating the two avoids reading equation-heavy display text aloud while also preventing conversational narration from cluttering the learner's notes. Visual requests may additionally return a recognized equation and candidate plot data. When handwriting is unclear, the prompt asks the model to express uncertainty rather than invent missing symbols.

Explanations can be simplified or regenerated. For an unchanged selection, the backend first checks SQLite using a hash that includes the selected content and its document, page, canvas, shape, and image context. A match restores both the written answer and its voice explanation without another language-model request. Speech has a second text-hash cache, so replaying the same voice explanation can reuse the previously generated audio as well. When a learner asks several questions in quick succession, the requests remain visible in a queue and are handled in order. Each request keeps the selection that was active when it was submitted, even if the learner continues writing or clicks elsewhere. Audio follows the same order, and replaying a completed explanation uses the saved recording instead of generating it again.
...
The backend is a Bun and Hono TypeScript service using Bun's SQLite interface, pdf-parse, pdf-lib, Sharp, and Kokoro JS. The frontend is a React and TypeScript application using PDF.js, react-pdf, tldraw, graphology, Sigma, and Framer Motion. OpenRouter is used through configurable endpoints for generation, vision, embeddings, and Fish Audio speech. The default configuration uses free model endpoints and keeps the language, vision, embedding, and speech choices independently configurable.

The persistence layer includes tables for documents, page text, chunks, embeddings, notes, explanations, graph concepts, graph edges, manual groups, speech-cache entries, and provider telemetry. Explanation history stores the display answer, the voice-specific explanation, and the classified intent together. Database startup performs schema initialization and lightweight migrations, including the addition of these newer fields to an existing local database. Destructive operations remove affiliated files, indexes, graph edges, explanations, and browser recovery data so deleted study material does not reappear after reload.

The application exposes separate routes for documents, canvases, notes, explanations, retrieval, search, graph operations, and speech. Language requests carry an ordered list of free OpenRouter models. If the preferred model is unavailable, rate-limited, or rejected by its provider, OpenRouter can move through that list before using its free dynamic router. Models do not always name a category exactly as requested; for example, a model may return ``conceptual explanation'' instead of ``theory.'' ScholarLM maps these reasonable variations to one of its four teaching styles and uses the general style for anything unfamiliar. This prevents a useful answer from being lost because of a minor classification difference. Fish Audio speech is requested separately through OpenRouter and cached as MP3 when available. If that request fails, the backend produces WAV audio with local Kokoro; if neither server path succeeds, the browser's SpeechSynthesis API remains the final fallback.

The production package uses Docker Compose with a Bun backend and an Nginx frontend on a shared internal network. Nginx serves the web application and forwards \texttt{/api} requests to the backend. SQLite lives with the backend and is kept in a Docker volume, so a separate database container is unnecessary and recreating the application containers does not remove study data. During local development, the Bun frontend and backend run directly and Nginx is not involved. Navigation is committed immediately rather than being deferred by React Router. This matters in a canvas-heavy application: without it, the address bar could show the Notes route while the previous document workspace was still on screen.
...
\midrule
Text generation & Google Gemma 4 26B A4B & Ordered free models: Gemma 4 31B, GPT-OSS 20B, Nemotron Nano 9B V2, then the OpenRouter free router.\\
Vision and handwriting & Google Gemma 4 26B A4B & The same ordered route, with OpenRouter selecting a compatible free endpoint.\\
Document embeddings & NVIDIA Llama Nemotron Embed VL 1B V2 & Re-indexing can be retried with the configured embedding provider; unavailable embeddings disable semantic retrieval until indexing succeeds.\\
Speech synthesis & Fish Audio S2.1 Pro through OpenRouter & Local Kokoro JS, then browser SpeechSynthesis when server audio is unavailable.\\
...
The current primary language model is Gemma 4 26B, while the embedding model is NVIDIA Llama Nemotron Embed VL 1B v2. The default \texttt{OPENROUTER\_ROUTING\_MODELS} value continues with Gemma 4 31B, GPT-OSS 20B, Nemotron Nano 9B V2, and finally \texttt{openrouter/free}; every entry in this chain uses a free endpoint. The order remains configurable because free-model availability changes over time. The backend records provider operations and failures for diagnosis, preserves the learner's active selection after a failed request, and reuses valid cached work whenever possible. In practice, this makes a temporary provider failure less disruptive without hiding the source or status of the generated material.
...
Explanation workflow & Text, handwriting, and screenshot selections use one of four teaching styles; written and spoken outputs are separated, queued, cached, and restored together.\\
Navigation & Deep routes are served through Nginx in Docker, while synchronous route commits prevent stale document UI after the URL changes.\\
...
Speech & Fish Audio MP3 generation, Kokoro WAV fallback, text-hash caching, playback controls, and browser fallback are available.\\
...
The automated backend suite checks model fallback selection, spoken versions of mathematical symbols, the separation between displayed and spoken explanations, and variations in model-produced category names. One regression test recreates the failure in which a useful explanation was rejected because the model called it a ``conceptual explanation.'' Both applications are type-checked, the production frontend is built, and the Docker version is checked through the Notes page and backend health route. A broader repeatable evaluation set is still needed for top-page retrieval accuracy, grounded versus unsupported answers, graph quality, equation transcription, cold and warm latency, speech fallback, and deletion during active ingestion.
