...
The application exposes separate routes for documents, canvases, notes, explanations, retrieval, search, graph operations, and speech. Language requests carry an ordered list of free OpenRouter models. If the preferred model is unavailable, rate-limited, or rejected by its provider, OpenRouter can move through that list before using its free dynamic router. Fish Audio speech is requested separately through OpenRouter and cached as MP3 when available. If that request fails, the backend produces WAV audio with local Kokoro; if neither server path succeeds, the browser's SpeechSynthesis API remains the final fallback.
...
Explanation workflow & Text, handwriting, and screenshot selections are classified by intent; written and spoken outputs are separated, cached, and restored together.\\
