import { NOTE_STORAGE_PREFIX } from "./constants";
...
export function getNoteStorageKey(noteId: string): string {
  return `${NOTE_STORAGE_PREFIX}:${noteId}`;
}
export function saveLocalNoteDraft(draft: LocalNoteDraft): void {
  localStorage.setItem(getNoteStorageKey(draft.noteId), JSON.stringify(draft));
}
export function getLocalNoteDraft(noteId: string): LocalNoteDraft | null {
  try {
    const raw = localStorage.getItem(getNoteStorageKey(noteId));
    if (!raw) return null;
    const value: unknown = JSON.parse(raw);
    if (!value || typeof value !== "object") return null;
    const d = value as Partial<LocalNoteDraft>;
...
export function chooseNewestNoteSource({
  ...
  const localTime = Date.parse(local.updatedAt),
    serverTime = Date.parse(server.updatedAt);
  return Number.isFinite(localTime) && localTime > serverTime
    ? "local"
    : "server";
}
